...
public class CommandC implements Runnable {
	...
	public CommandC(FtpClient ftpClient, String hostname, int nPort) throws Exception {
	...
	public void run() {
		try {
			...
			do {
				...
				else if ((tokens[0].equalsIgnoreCase("ls") || tokens[0].equalsIgnoreCase("pwd")
						|| tokens[0].equalsIgnoreCase("quit")) && tokens.length != 1)
					System.out.println("Incorrect number of arguments");
				else if ((tokens[0].equalsIgnoreCase("delete") || tokens[0].equalsIgnoreCase("mkdir"))
						&& tokens.length != 2)
					System.out.println("Incorrect number of arguments");
				...
				if (tokens[0].equalsIgnoreCase("get")) {
					if (tokens.length > 2) {
						if (tokens[2].equals("&")) {
							...
							(new Thread(new GetBg(ftpClient, hostname, nPort, tokens, filePath, filePathClient)))
									.start();
				 ...
				 *************************************/
					...
					ftpClient.transferIN(serverPath.resolve(tokens[1]), terminateID);
					// prints different Command ID here
					// ready to write
					br.readLine();
					Thread.sleep(100);
